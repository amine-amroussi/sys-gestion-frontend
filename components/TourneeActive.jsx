...
const TourneeActive = () => {
  ...
  const calculateEstimatedRevenue = (tripProducts, tripWastes = [], tripCharges = []) => {
    const productRevenue = tripProducts.reduce((total, product) => {
      const productData = products.find(p => Number(p.id) === Number(product.product)) || {};
    ...
    const wasteCost = tripWastes.reduce((total, waste) => {
      const productData = products.find(p => Number(p.id) === Number(waste.product)) || {};
      const priceUnite = productData.priceUnite || 0;
      return total + (waste.qtt || 0) * priceUnite;
    }, 0);

    const totalCharges = tripCharges.reduce((total, charge) => total + (charge.amount || 0), 0);

    return productRevenue - wasteCost - totalCharges;
  };

  const calc = (tripWastes = [], tripCharges = []) => {
    const productRevenue = formDataStep1.tripProducts.reduce((total, product) => {
      const tripProduct = tripDetails.TripProducts.find(
        tp => Number(tp.product) === Number(product.product_id)
      );
      const productData = products.find(p => Number(p.id) === Number(product.product_id)) || {};
      const capacityByBox = productData.capacityByBox || 0;
      const priceUnite = productData.priceUnite || 0;
      const qttVendu =
        ((tripProduct?.qttOut || 0) - (product.qttReutour || 0)) * capacityByBox +
  ...
  return (
    <div className="container mx-auto ">
        ...
        {activeTrips.map(trip => (
          ...
          >
            <div className="flex justify-between items-center border-b border-gray-200 pb-2 mb-2">
              <h4 className="text-lg font-semibold text-gray-800">Tournée #{trip.id}</h4>
              <span className="text-sm text-gray-500">{new Date(trip.date).toLocaleDateString()}</span>
            </div>
      ...
      {isDetailsModalOpen && tripDetails && !isFinishStep1Open && !isFinishStep2Open && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-gray-200 pb-2 mb-4">
              <h5 className="text-xl font-semibold text-gray-800">Détails de la Tournée #{selectedTripId}</h5>
              <button
                onClick={closeModal}
            ...
            <div className="space-y-4 text-sm text-gray-700">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><span className="font-medium">Camion:</span> {tripDetails.TruckAssociation?.matricule || "N/A"}</div>
                <div><span className="font-medium">Conducteur:</span> {tripDetails.DriverAssociation?.name || "N/A"}</div>
                <div><span className="font-medium">Vendeur:</span> {tripDetails.SellerAssociation?.name || "N/A"}</div>
                <div><span className="font-medium">Assistant:</span> {tripDetails.AssistantAssociation?.name || "N/A"}</div>
                <div><span className="font-medium">Zone:</span> {tripDetails.zone}</div>
                <div><span className="font-medium">Date:</span> {new Date(tripDetails.date).toLocaleDateString()}</div>
              </div>
              {tripDetails.TripProducts && tripDetails.TripProducts.length > 0 && (
                <div>
                  <h6 className="text-md font-semibold text-gray-800">Produits Sortis</h6>
                  <table className="w-full text-sm border-collapse">
                    ...
                    <tbody>
                      {tripDetails.TripProducts.map((product, index) => {
                        const productData = products.find(p => Number(p.id) === Number(product.product)) || {};
                        const capacityByBox = productData.capacityByBox || 0;
                        const totalUnits = (product.qttOut || 0) * capacityByBox + (product.qttOutUnite || 0);
                        return (
                          <tr key={index} className="border-b hover:bg-gray-50">
                            <td className="p-2">{productData.designation || "N/A"}</td>
                    ...
                    <tfoot>
                      <tr className="border-t font-semibold">
                        ...
                        <td className="p-2">{tripDetails.TripProducts.length} produits</td>
                        <td className="p-2"></td>
                        <td className="p-2">
                          {tripDetails.TripProducts.reduce((sum, product) => {
                            const productData = products.find(p => Number(p.id) === Number(product.product)) || {};
                            const capacityByBox = productData.capacityByBox || 0;
                            return sum + ((product.qttOut || 0) * capacityByBox + (product.qttOutUnite || 0));
                          }, 0)}
                        </td>
                      </tr>
                      <tr className="border-t font-semibold">
              ...
              {tripDetails.TripBoxes && tripDetails.TripBoxes.length > 0 && (
                <div>
                  <h6 className="text-md font-semibold text-gray-800">Boîtes Sorties</h6>
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="border-b bg-gray-100">
                        <th className="text-left p-2">Désignation</th>
                    ...
                    <tbody>
                      {tripDetails.TripBoxes.map((box, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          <td className="p-2">{box.BoxAssociation?.designation || "N/A"}</td>
                    ...
                    <tfoot>
                      <tr className="border-t font-semibold">
                        <td className="p-2">Total</td>
                        <td className="p-2">{tripDetails.TripBoxes.length} boîtes</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
      ...
      {isFinishStep1Open && tripDetails && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-gray-200 pb-2 mb-4">
              <h5 className="text-xl font-semibold text-gray-800">Terminer la Tournée #{selectedTripId} - Étape 1</h5>
              <button
                onClick={closeModal}
                className="text-gray-600 hover:text-red-500 text-xl font-bold"
              >
      ...
      {isFinishStep2Open && tripDetails && formDataStep1 && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-gray-200 pb-2 mb-4">
              <h5 className="text-xl font-semibold text-gray-800">Résumé de la Tournée #{selectedTripId} - Étape 2</h5>
              <button
                onClick={closeModal}
                className="text-gray-600 hover:text-red-500 text-xl font-bold"
              >
            ...
            <div className="space-y-4 text-sm text-gray-700">
              ...
              {formDataStep1.tripProducts && formDataStep1.tripProducts.length > 0 && (
                <div>
                  <h6 className="text-md font-semibold text-gray-800">Produits Retournés</h6>
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="border-b bg-gray-100">
                        <th className="text-left p-2">Désignation</th>
                        <th className="text-left p-2">Qté Sortie (Caisses)</th>
                        <th className="text-left p-2">Qté Sortie (Unités)</th>
                        <th className="text-left p-2">Qté Retour (Caisses)</th>
                    ...
                    <tbody>
                      {formDataStep1.tripProducts.map((product, index) => {
                        const tripProduct = tripDetails.TripProducts.find(
                          p => Number(p.product) === Number(product.product_id)
                        );
                        const productData = products.find(p => Number(p.id) === Number(product.product_id)) || {};
                        const capacityByBox = productData.capacityByBox || 0;
                        const priceUnite = productData.priceUnite || 0;
                        ...
                        return (
                          <tr key={index} className="border-b hover:bg-gray-50">
                            <td className="p-2">{productData.designation || "N/A"}</td>
                            <td className="p-2">{tripProduct?.qttOut || 0}</td>
                            <td className="p-2">{tripProduct?.qttOutUnite || 0}</td>
                            <td className="p-2">{product.qttReutour || 0}</td>
                            <td className="p-2">{product.qttReutourUnite || 0}</td>
                            <td className="p-2">{qttVendu}</td>
                            <td className="p-2">{(qttVendu * priceUnite).toFixed(2)} MAD</td>
                          </tr>
                        );
                      })}
                    </tbody>
                    <tfoot>
                      <tr className="border-t font-semibold">
                        <td className="p-2">Total</td>
                        <td className="p-2">{tripDetails.TripProducts.reduce((sum, p) => sum + (p.qttOut || 0), 0)}</td>
                        <td className="p-2">{tripDetails.TripProducts.reduce((sum, p) => sum + (p.qttOutUnite || 0), 0)}</td>
                        <td className="p-2">{formDataStep1.tripProducts.reduce((sum, p) => sum + (p.qttReutour || 0), 0)}</td>
                        <td className="p-2">{formDataStep1.tripProducts.reduce((sum, p) => sum + (p.qttReutourUnite || 0), 0)}</td>
                        <td className="p-2">
                          {formDataStep1.tripProducts.reduce((sum, product) => {
                            const tripProduct = tripDetails.TripProducts.find(
                              p => Number(p.product) === Number(product.product_id)
                            );
                            const productData = products.find(p => Number(p.id) === Number(product.product_id)) || {};
              ...
              {formDataStep1.tripBoxes && formDataStep1.tripBoxes.length > 0 && (
                <div>
                  <h6 className="text-md font-semibold text-gray-800">Boîtes Retournées</h6>
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="border-b bg-gray-100">
                        <th className="text-left p-2">Désignation</th>
                    ...
                    <tbody>
                      {formDataStep1.tripBoxes.map((box, index) => {
                        const tripBox = tripDetails.TripBoxes.find(b => Number(b.box) === Number(box.box_id));
                        return (
                          <tr key={index} className="border-b hover:bg-gray-50">
                            <td className="p-2">{tripBox?.BoxAssociation?.designation || "N/A"}</td>
                    ...
                    <tfoot>
                      <tr className="border-t font-semibold">
                        <td className="p-2">Total</td>
                        <td className="p-2">{tripDetails.TripBoxes.reduce((sum, b) => sum + (b.qttOut || 0), 0)}</td>
                        <td className="p-2">{formDataStep1.tripBoxes.reduce((sum, b) => sum + (b.qttIn || 0), 0)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
              ...
              {formDataStep1.tripWastes && formDataStep1.tripWastes.length > 0 && (
                <div>
                  <h6 className="text-md font-semibold text-gray-800">Déchets</h6>
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="border-b bg-gray-100">
                    ...
                    <tbody>
                      {formDataStep1.tripWastes.map((waste, index) => {
                        const productData = products.find(p => Number(p.id) === Number(waste.product)) || {};
                        const priceUnite = productData.priceUnite || 0;
                        return (
                          <tr key={index} className="border-b hover:bg-gray-50">
                            ...
                            <td className="p-2">{(waste.qtt * priceUnite).toFixed(2)} MAD</td>
                          </tr>
                        );
                      })}
                    </tbody>
                    <tfoot>
                      <tr className="border-t font-semibold">
                        <td className="p-2">Total</td>
                        <td className="p-2"></td>
                        <td className="p-2">{formDataStep1.tripWastes.reduce((sum, waste) => sum + (waste.qtt || 0), 0)}</td>
                        <td className="p-2">
                          {formDataStep1.tripWastes.reduce((sum, waste) => {
                            const productData = products.find(p => Number(p.id) === Number(waste.product)) || {};
              ...
              {formDataStep1.tripCharges && formDataStep1.tripCharges.length > 0 && (
                <div>
                  <h6 className="text-md font-semibold text-gray-800">Charges</h6>
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="border-b bg-gray-100">
                        <th className="text-left p-2">Type</th>
                    ...
                    <tbody>
                      {formDataStep1.tripCharges.map((charge, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-2">{(charge.amount || 0).toFixed(2)} MAD</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr className="border-t font-semibold">
                        <td className="p-2">Total</td>
                        <td className="p-2">{formDataStep1.tripCharges.reduce((sum, charge) => sum + (charge.amount || 0), 0).toFixed(2)} MAD</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
              ...
              <div>
                <h6 className="text-md font-semibold text-gray-800">Résumé Financier</h6>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border p-4 rounded-md bg-gray-50">
                  <div><span className="font-medium">Revenu Brut Estimé:</span> {calculateTotalProductAmount()} MAD</div>
                  <div><span className="font-medium">La commission de vente:</span> {(calculateTotalProductAmount() * 0.008).toFixed(2)} MAD</div>
                  <div>
                    ...
                    {formDataStep1.tripWastes.length > 0 && (
                      <span className="block text-xs text-gray-500">
                        (Quantité Totale: {formDataStep1.tripWastes.reduce((sum, waste) => sum + (waste.qtt || 0), 0)} unités)
                      </span>
                    )}
                  </div>
                  <div><span className="font-medium">Total Charges:</span> {formDataStep1.tripCharges.reduce((total, charge) => total + (charge.amount || 0), 0).toFixed(2)} MAD</div>
                  <div><span className="font-medium">Revenu Net Estimé (Attendu):</span> {calc(formDataStep1.tripWastes, formDataStep1.tripCharges)} MAD</div>
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700">Montant Reçu (MAD)</label>
                <input
                  type="number"
